...
import { o as id } from "./utils-BOIfZi7B.mjs";
import { t as aiProvider } from "./ai-provider-2njZqQPi.mjs";
import { a as custom, c as object, r as array, t as _enum, u as string } from "../_libs/zod.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/diligence-DZiMoM8y.js
var criticalClaimIds = /* @__PURE__ */ new Set([
	"sales",
	"rent",
	"tax",
	"zoning",
...
var modelMemoSchema = object({
	memo: string().min(80).max(1400),
	risks: array(object({
		label: string().max(120),
		severity,
		detail: string().max(400)
	})).max(5),
	requests: array(string().max(240)).max(8)
});
function source(id, label, kind, status, detail) {
	return {
		id,
		label,
		kind,
...
function buildEvidence(dossier, neighborhood) {
	const sources = [
	...
	if (neighborhood?.census) sources.push(source("census", `${neighborhood.census.source} ${neighborhood.census.vintage}`, "public", "available", `ZIP-level profile for ${neighborhood.census.geography}.`));
	return {
		sources,
		claims: [
			{
			...
			},
			{
				...
				confidence: neighborhood?.census ? .72 : null,
...
function baselineRun(dossier, neighborhood) {
	...
	const risks = [...dossier.risks.slice(0, 3), ...missing.slice(0, 3).map((claim) => ({
		label: claim.label,
		severity: "high",
		detail: claim.detail
	}))];
...
var createDiligenceRun_createServerFn_handler = createServerRpc({
	id: "728a3c0b896842cd48d621097adcbe79ab0bf3c1aa6313ef75e7240505338db5",
	name: "createDiligenceRun",
	filename: "src/lib/server/diligence.ts"
}, (opts) => createDiligenceRun.__executeServer(opts));
var createDiligenceRun = createServerFn({ method: "POST" }).validator((input) => ({
...
})).handler(createDiligenceRun_createServerFn_handler, async ({ data }) => {
	const run = baselineRun(data.dossier, data.neighborhood);
	const factualClaims = run.claims.filter((claim) => claim.status === "verified" || claim.status === "inferred").map((claim) => `${claim.label}: ${claim.value} (${claim.status}; ${claim.detail})`).join("\n");
	const ai = await aiProvider.runStructured({
		mode: data.mode,
		profile: "smart",
		system: "You are an acquisition-diligence analyst. Use only the supplied evidence. Do not invent facts, comps, prices, permits, hazard conditions, or claims. Return concise JSON. Every recommendation must preserve the stated uncertainty.",
		prompt: `Property: ${data.dossier.pin.address}, ${data.dossier.pin.city}, ${data.dossier.pin.state}\n\nEvidence:\n${factualClaims}\n\nCritical gaps:\n${run.claims.filter((claim) => claim.status === "missing").map((claim) => `- ${claim.label}: ${claim.detail}`).join("\n")}`,
	...
	return {
		...
		run: {
			...
			requests: [.../* @__PURE__ */ new Set([...ai.value.requests, ...run.requests])].slice(0, 8),
			modelRun: ai.provenance
		}
	};
});
//#endregion
export { createDiligenceRun_createServerFn_handler };
