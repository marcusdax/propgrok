import type { FeatureKey, Incentive, MetroProfile } from "./types";
...
export const FEATURES: {
  ...
  { key: "garage_door", label: "Garage door", roiPct: 93.8, costLow: 1400, costHigh: 4200, market: "$2.8B" },
  { key: "siding", label: "Siding", roiPct: 80.1, costLow: 10000, costHigh: 25000, market: "$9.7B" },
  { key: "roof", label: "Roofing", roiPct: 68.2, costLow: 8000, costHigh: 25000, market: "$28.5B" },
  { key: "windows", label: "Windows", roiPct: 67.4, costLow: 5000, costHigh: 15000, market: "$12.4B" },
  { key: "paint", label: "Exterior paint", roiPct: 55.2, costLow: 3000, costHigh: 7200, market: "$6.2B" },
  { key: "landscaping", label: "Curb landscape", roiPct: 48.0, costLow: 1800, costHigh: 8500, market: "$4.1B" },
];

export const FEATURE_STYLES: Record<FeatureKey, { id: string; label: string; prompt: string }[]> = {
  garage_door: [
    { id: "carriage", label: "Carriage black", prompt: "a modern matte black carriage-style garage door with decorative hardware and frosted glass lites" },
    { id: "glass", label: "Full-view glass", prompt: "a contemporary full-view aluminum and frosted-glass garage door, slim black frames" },
    { id: "wood", label: "Warm cedar", prompt: "a horizontal cedar-plank garage door with a warm natural stain and black handles" },
    { id: "white", label: "Classic raised panel", prompt: "a crisp white raised-panel steel garage door, clean and well-proportioned" },
  ],
  roof: [
    { id: "arch", label: "Architectural shingle", prompt: "a new architectural asphalt-shingle roof in charcoal gray, clean ridges and even courses" },
    { id: "metal", label: "Standing-seam metal", prompt: "a standing-seam metal roof in dark bronze, precise seams, no rust" },
    { id: "slate", label: "Synthetic slate", prompt: "a synthetic slate roof in deep graphite with crisp edges" },
  ],
  windows: [
    { id: "black", label: "Black framed", prompt: "new black-framed vinyl windows with crisp grids, replacing old frames in the same openings" },
    { id: "white", label: "White energy", prompt: "new white energy-efficient windows with low-E glass, same openings and trim" },
  ],
  siding: [
    { id: "board", label: "Board and batten", prompt: "fresh vertical board-and-batten siding in warm greige, replacing worn siding" },
    { id: "lap", label: "James Hardie lap", prompt: "new fiber-cement lap siding in a muted sage, tight joints, no peeling" },
    { id: "brick", label: "Painted brick", prompt: "the brick facade freshly limewashed in a warm off-white, mortar intact" },
  ],
  paint: [
    { id: "ink", label: "Ink body / cream trim", prompt: "the house freshly painted: deep ink body, cream trim, and a sage front door, no peeling" },
    { id: "sand", label: "Warm sand", prompt: "the house freshly painted in a warm sand color with white trim and black accents" },
    { id: "olive", label: "Olive contemporary", prompt: "the house freshly painted in a muted olive with black window frames and a natural wood door" },
  ],
  landscaping: [
    { id: "native", label: "Native drought", prompt: "a freshly landscaped front yard with native drought-tolerant plantings, gravel paths, and a tidy lawn edge" },
    { id: "lush", label: "Lush traditional", prompt: "a lush, freshly edged lawn, boxwoods, and seasonal flowers framing the facade" },
  ],
};

export const HAIL_BELT = new Set([
  "TX", "OK", "KS", "NE", "CO", "MO", "AR", "IA", "SD", "WY", "NM", "IL",
]);

export const METROS: MetroProfile[] = [
  {
    id: "dfw",
...
export const FALLBACK_METRO: MetroProfile = {
  id: "us",
  name: "United States",
  state: "US",
  medianValue: 362000,
...
export function metroFor(city: string, state: string): MetroProfile {
  ...
  if (c.includes("dallas") || c.includes("fort worth") || c.includes("arlington"))
    return METROS.find((m) => m.id === "dfw")!;
  if (c.includes("austin")) return METROS.find((m) => m.id === "aus")!;
  if (c.includes("tampa") || c.includes("st. pete") || c.includes("st petersburg"))
    return METROS.find((m) => m.id === "tpa")!;
  if (c.includes("denver") || c.includes("aurora") || c.includes("boulder"))
    return METROS.find((m) => m.id === "den")!;
  if (c.includes("phoenix") || c.includes("scottsdale") || c.includes("mesa") || c.includes("tempe"))
    return METROS.find((m) => m.id === "phx")!;
  if (c.includes("miami") || c.includes("fort lauderdale") || c.includes("boca"))
    return METROS.find((m) => m.id === "mia")!;
  if (c.includes("chicago")) return METROS.find((m) => m.id === "chi")!;
  if (c.includes("los angeles") || c.includes("long beach") || c.includes("pasadena"))
    return METROS.find((m) => m.id === "lax")!;
  ...
  return { ...FALLBACK_METRO, state: state || "US", name: city ? `${city} metro` : FALLBACK_METRO.name };
}

export function incentivesFor(state: string, metro: MetroProfile): Incentive[] {
  const s = state.toUpperCase();
  const list: Incentive[] = [
    {
