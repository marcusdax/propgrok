...
type CensusProfile = NonNullable<NeighborhoodIntel["census"]>;

async function fetchCensusProfile(postcode: string): Promise<CensusProfile | null> {
  ...
  try {
    ...
    const bachelorPlus = ["B15003_022E", "B15003_023E", "B15003_024E", "B15003_025E"]
      .map(value)
      .reduce<number>((sum, current) => sum + (current ?? 0), 0);
    ...
    return {
      ...
      medianIncome,
      medianAge,
      bachelorPlusPct: adults && bachelorPlus != null ? Math.round((bachelorPlus / adults) * 1000) / 10 : null,
      ownerOccupancyPct: tenureTotal && ownerOccupied != null ? Math.round((ownerOccupied / tenureTotal) * 1000) / 10 : null,
      householdSize: null,
    };
  } catch {
    return null;
...
export const analyzeNeighborhood = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const schoolHighlights = data.amenities
      .filter((a) => isEducation(a.kind))
      .sort((a, b) => a.distanceM - b.distanceM)
      .slice(0, 5)
      .map(({ name, kind, distanceM }) => ({ name, kind, distanceM }));
    const amenityHighlights = data.amenities
      .filter((a) => !isEducation(a.kind))
      .sort((a, b) => a.distanceM - b.distanceM)
      .slice(0, 3);
    const amenityDensity = (data.amenities.length / Math.max(0.1, Math.PI * (1200 / 1609.344) ** 2)).toFixed(1);
    ...
    const intel: NeighborhoodIntel = {
      ...
      investorRead:
        "This is an access-and-context screen, not a valuation. The strongest local signal is the combination of mapped family amenities and nearby services; the next decision should be driven by verified rent, tax, insurance, condition, and comparable-sale evidence.",
      schoolsNote:
        schoolCount > 0
          ? `${schoolCount} school POIs returned in this radius. Ratings and attendance boundaries require official district verification.`
          : "No mapped school buildings or nodes returned in this radius — widen the radius or verify district maps.",
