...
export async function ensureFreeLLMRunning(): Promise<{ available: boolean; port?: number }> {
  ...
  try {
    // Start the server in the background (server-only Node API)
...
export type MapIntensityResponse = {
  ...
  signals: Array<{ label: string; value: string; detail: string; tone: "positive" | "caution" | "neutral" }>;
  coverageNote: string;
    ...
    signals: [
      {
        ...
        value: usdCompact(medianIncome),
        detail: "Synthetic median-income estimate for visual modeling only; verify with ACS or licensed tract data.",
        tone: "neutral",
      },
      {
        label: "Education profile",
        value: `${educationBachelorPlus}% bachelor+`,
        detail: "Synthetic education estimate; do not use as a demographic fact without a cited source.",
        tone: "neutral",
      },
      {
        label: "Risk watch",
        value: `${Math.max(floodRisk, wildfireRisk, hailRisk, windRisk)}% max modeled hazard`,
        detail: "Highest modeled hazard channel should trigger parcel-level insurance and hazard verification.",
        tone: "caution",
      },
      {
        label: "Opportunity posture",
        value: investmentScore >= 70 ? "Investigate" : "Underwrite carefully",
        detail: "Opportunity score is a synthetic screening heuristic, not a return forecast.",
        tone: investmentScore >= 70 ? "positive" : "caution",
      },
    ],
    coverageNote: `Synthetic overlay generated for a ${radiusM} m radius. No live Census, school, zoning, rent, or sales source was attached to this response.`,
    source: "fallback",
  };
}
